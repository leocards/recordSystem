...
import AddedTags from '@/Components/Aside/TagOffice/AddedTags.vue';
...
const getSearches = async (val) => {
    ...
    let getSearch = await axios.post(route('users.search', [val, props.excludes]))
    ...
    >
        <div class="p-4 pr-3.5 relative">
            ...
            <div class="mb-4 text-lg">
                Add personnel to " {{ personnel.name }} "
            </div>

            <div v-if="!isNext">
                <div class="flex mb-3 pr-0.5">
                    <SearchInput :searchVal="searchValue" @handleSearch="debounces" searchLabels="personnel" />
                </div>

                <div class="flex flex-wrap mbt3 mb-5 border border-slate-700/20 rounded-lg p-1 max-h-[10rem] overflow-y-auto" v-if="addOfficeForm.personnel.length > 0">
                    <AddedTags 
                        v-for="(item, index) in addOfficeForm.personnel"
                        :index="index"
                        :item="item"
                        @handle-remove-office="removePersonnel(index)"
                    />
                </div>
            ...
            <div class="h-[25rem] flex flex-col overflow-y-hidden mt-5" v-else>
                <div class="grow overflow-y-auto pr-0.5 text-sm">
                    <div class="rounded-xl flex flex-col p-3 borde r border-slate-700/20 bg-gray-100 mt-2 text-sm" v-for="(item, index) in addOfficeForm.personnel" :key="index">
                        <div class="">
                            {{ item.name }}
                        </div>
            ...
            <div class="flex flex-row-reverse justify-between mt-3 pr-0.5">
                <div class="w-fit">
                    ...
                    <PrimaryButton
                        class="ml-3"
                        :disabled="addOfficeForm.personnel.length === 0 || addOfficeForm.processing"
                        @click="submit"
                    >
                        Add personnel
                    </PrimaryButton>
                ...
                <button 
                    ...
                    v-if="addOfficeForm.personnel.length !== 0"
                    @click="isNext = !isNext"
                >
                    {{ isNext ? 'Back':'Next' }}
                </button>
